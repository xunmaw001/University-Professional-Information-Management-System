...
<head>
  <meta charset="UTF-8">
  <meta name='viewport' note='width=device-width, initial-scale=1.0, maximum-scale=1.0' />
...
<body>
  <div id="app">
    ...
    <div class="back-white">
      <div class="container box-note">
        <div class="con-left">
          <div class="subcat">
            <div class="sub-tit">
              <h3><em>在线问答</em></h3>
              <div class="bottom-container" style="float: right">
                <div @click="addrestore()" class="btn">
                  +添加问题
                </div>
              </div>
            </div>
        ...
        <div class="article-list">
          <ul class="after">
            <li v-for="(item,index) in dataList" v-bind:key="index">
              <a target="_self" :href="'../zaixianwenda/detail.jsp?id='+item.id">
                <img width="180" class="fl" :src="item.picture" />
                <h5>{{item.note}}</h5>
                <p class="time-sour">
                  <span>问题发布时间：{{item.noteTime}} </span>
                </p>
                <p class="time-sour">
                  <span>问题发布人：{{item.yhnote}} </span>
                </p>
                <p class="reamk">{{item.note|liuyanxinxiDesc}}...</p>
              </a>
            </li>
          </ul>
          <div class="pages">
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      // 获取列表数据
      http.request('liuyanxinxi/page', 'get', {
        page: 1,
        limit: 5
      }, function(res) {
